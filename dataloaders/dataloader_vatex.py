...
class VATEX_DataLoader(Dataset):
    ...
    def _get_text_with_aug(self, video_id, query):
        ...
        input_ids, input_mask, segment_ids = _tokenize_sentence(query)
        ...
        if self.aug_data is not None and video_id in self.aug_data:
            ...
            if isinstance(aug_payload, dict):
                for _cap_key, cap_data in aug_payload.items():
                    if isinstance(cap_data, dict) and cap_data.get("original", "") == query:
    ...
    def __getitem__(self, idx):
        ...
        if self.aug_data is not None:
            pairs_text, pairs_mask, pairs_segment = self._get_text_with_aug(video_id, query)
            choice_video_ids = [video_id]
        else:
